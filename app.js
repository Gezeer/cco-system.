...
function aplicarRestricoesPerfil() {
  ...
  if (usuarioLogado.usuario === "cco" || usuarioLogado.perfil === "Operador") {
    esconderElemento("btnExportar");
    esconderElemento("btnLimpar");
...
function esconderBotaoMenu(texto) {
  document.querySelectorAll(".menu button").forEach(botao => {
    if (botao.innerText.includes(texto)) botao.style.display = "none";
  });
}
...
function abrirTelaInicial(idTela) {
  ...
  document.querySelectorAll(".menu button").forEach(botao => botao.classList.remove("active"));
}
...
async function importarPlanilhas(evento) {
  ...
  try {
    ...
    const salvou = await salvarPlanilhaNoBanco(arquivos.map(a => a.name).join(", "));
    ...
    if (salvou) alert("Planilha importada e salva com sucesso!");
...
function ehCampoDataHora(coluna) {
  ...
  return nome.includes("data") || nome.includes("inicio") || nome.includes("fim") || nome.includes("termino");
}
...
function formatarDataHoraBR(valor) {
  ...
  if (typeof valor === "number") {
    ...
    if (hora === "00" && minuto === "00") return `${dia}/${mes}/${ano}`;
...
function gerarPainelExecutivo() {
  ...
  painelExecutivo = painel.dadosNormalizados.map(item => {
    ...
    const acumulado = abaServico
      ? calcularAcumuladoPorServico(servico, abaServico.dadosNormalizados)
      : numero(item.acumulado_mes || item.acumulado_no_mes || item.executado || 0);
    ...
    return {
      ...
      dias_acumulados: numero(
        item.dias_acumulados ||
        item.dia_acumulado ||
        item.dias_acumulado ||
        item.dias_acumulado_mes ||
        item.dias_executados ||
        item.dias_operados ||
        item.qtd_dias ||
        item.quantidade_de_dias ||
        item.dias ||
        0
      ),

      total_dias_mes: numero(
        item.total_dias_mes ||
        item.total_de_dias_mes ||
        item.total_dias_do_mes ||
        item.dias_no_mes ||
        item.dias_mes ||
        item.quantidade_dias_mes ||
        item.qtd_dias_mes ||
        item.total_dias ||
        0
      ),
...
function renderTabelaExecutiva() {
  ...
  if (!painelExecutivo.length) {
    tabela.innerHTML = `<tr><td colspan="10">Importe uma planilha para visualizar os dados.</td></tr>`;
    return;
  }
...
/* CRIAR CARD OCULTANDO ZERO */
function criarCard(titulo, valor, subtitulo, ocultarZero = true) {
  const valorNumerico = numero(
    String(valor)
      .replace("t", "")
      .replace("%", "")
  );

  if (ocultarZero && valorNumerico === 0) return "";
...
function renderDetalheServico(codigo) {
  ...
  const diasAcumulados = dadosPainel ? numero(dadosPainel.dias_acumulados) : 0;

  const totalEquipes = aba.dadosNormalizados.reduce(
    (soma, item) => soma + numero(item.equipe || item.qtd_equipe || item.qdt_equipe || item.equipes || 0),
    0
  );

  const totalPeso = aba.dadosNormalizados.reduce(
    (soma, item) => soma + numero(item.peso_t || item.peso || item.peso_total || item.tonelada || item.toneladas || item.executado || 0),
    0
  );

  const totalKm = aba.dadosNormalizados.reduce(
    (soma, item) => soma + numero(item.km || item.km_total || item.km_executado || item.quilometragem || 0),
    0
  );

  const totalViagens = aba.dadosNormalizados.reduce(
    (soma, item) => soma + numero(item.viagens || item.qtd_viagem || item.qtd_viagens || item.quantidade_de_viagens || 0),
    0
  );

  let executado = 0;

  if (["P3", "P7", "P8", "P10", "P11"].includes(codigo)) {
    executado = diasAcumulados > 0
      ? Math.round(totalEquipes / diasAcumulados)
      : 0;
  } else if (["P1", "P4", "P12"].includes(codigo)) {
    executado = totalPeso;
  } else {
    executado = dadosPainel ? numero(dadosPainel.acumulado_mes) : 0;
  }
  ...
  detalhe.innerHTML = `
    <section class="cards">
      ${criarCard("Previsto", formatarNumero(previsto), "meta operacional")}

      ${criarCard(
        ["P1", "P4", "P12"].includes(codigo) ? "Executado (P)" : "Executado",
        ["P1", "P4", "P12"].includes(codigo) ? `${formatarNumero(executado)} t` : formatarNumero(executado),
        ["P1", "P4", "P12"].includes(codigo) ? "peso executado" : "acumulado realizado"
      )}

      ${criarCard("% Execução", `${formatarNumero(percentual)}%`, "índice operacional")}
      ${criarCard("Peso", `${formatarNumero(totalPeso)} t`, "toneladas")}
      ${criarCard("Viagens", formatarNumero(totalViagens), "operações")}
      ${criarCard("KM Executado", formatarNumero(totalKm), "quilometragem")}
      ${criarCard("Equipes", formatarNumero(totalEquipes), "equipes operacionais")}
    </section>
    ...
    <section class="section">
      ...
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${tbody || `<tr><td colspan="20">Nenhuma informação encontrada.</td></tr>`}
          </tbody>
        </table>
      </div>
    </section>
...
function renderFiltros() {
  ...
  if (filtroPrograma && filtroStatus) {
    ...
    filtroPrograma.innerHTML = programas.map(item =>
...
function aplicarFiltroMensal() {
  ...
  operacoes = operacoesOriginal.filter(item => {
    ...
    const [anoItem, mesItem] = item.data_normalizada.split("-");
...
function recalcularPainelPorFiltro() {
  painelExecutivo = painelExecutivoOriginal.map(item => {
    ...
    const dadosFiltrados = operacoes.filter(op => op.servico === servico);

    const acumulado = dadosFiltrados.reduce((soma, op) => {
      if (["P1", "P4", "P12"].includes(servico)) return soma + numero(op.peso);
      if (["P2.1", "P2.2"].includes(servico)) return soma + numero(op.viagens);
      if (["P5", "P6"].includes(servico)) return soma + numero(op.km);
      if (["P3", "P7", "P8", "P9", "P10", "P11"].includes(servico)) return soma + numero(op.equipe);
      return soma;
    }, 0);
...
function mostrarKPI(codigo, botao) {
  ...
  const servicoPainel = painelExecutivo.find(item => item.servico === codigo);
  const dadosServico = operacoes.filter(item => item.servico === codigo);
  ...
  if (!servicoPainel && !dadosServico.length) {
    area.innerHTML = `<div class="not-found">Nenhum dado encontrado para ${codigo}.</div>`;
    return;
  }

  const totalPeso = dadosServico.reduce((soma, item) => soma + numero(item.peso), 0);
  const totalViagens = dadosServico.reduce((soma, item) => soma + numero(item.viagens), 0);
  const totalKm = dadosServico.reduce((soma, item) => soma + numero(item.km), 0);
  const totalEquipe = dadosServico.reduce((soma, item) => soma + numero(item.equipe), 0);

  const produtividadeViagem = totalViagens > 0 ? totalPeso / totalViagens : 0;
  const produtividadeKm = totalKm > 0 ? totalPeso / totalKm : 0;
...
function gerarResumoExecutivoAutomatico() {
  ...
  const diasAcumulados = painelExecutivo.reduce((soma, item) => soma + numero(item.dias_acumulados), 0);

  const maiorExecucao = [...painelExecutivo].sort(
    (a, b) => numero(b.porcentagem_execucao) - numero(a.porcentagem_execucao)
  )[0];

  const menorExecucao = [...painelExecutivo].sort(
    (a, b) => numero(a.porcentagem_execucao) - numero(b.porcentagem_execucao)
  )[0];
...
async function salvarPlanilhaNoBanco(nomeArquivo = "Planilha Importada") {
  ...
  const { data: importacaoExistente, error: erroConsulta } = await banco
    .from("importacoes")
    .select("*")
  ...
  if (importacaoExistente && importacaoExistente.length > 0) {
    const continuar = confirm("Esta planilha já foi importada. Deseja importar novamente?");
    if (!continuar) {
...
async function carregarBanco() {
  ...
  preencherTexto("nomeArquivo", `${todasAsAbas.length} aba(s) carregada(s) do Supabase`);
}
...
async function carregarHistorico() {
  ...
  if (error) {
    console.error(error);
    tabela.innerHTML = `<tr><td colspan="7">Erro ao carregar histórico.</td></tr>`;
    return;
  }

  if (!data || !data.length) {
    tabela.innerHTML = `<tr><td colspan="7">Nenhuma importação encontrada.</td></tr>`;
    return;
  }
...
function calcularAcumuladoPorServico(servico, dados) {
  return dados.reduce((soma, item) => {
    if (["P1", "P4", "P12"].includes(servico)) {
      return soma + numero(item.peso_t || item.peso || item.peso_total || item.tonelada || item.toneladas || item.executado || 0);
    }

    if (["P2.1", "P2.2"].includes(servico)) {
      return soma + numero(item.viagens || item.qtd_viagem || item.qtd_viagens || item.quantidade_de_viagens || 0);
    }

    if (["P5", "P6"].includes(servico)) {
      return soma + numero(item.km || item.km_total || item.km_executado || item.quilometragem || 0);
    }

    if (["P3", "P7", "P8", "P9", "P10", "P11"].includes(servico)) {
      return soma + numero(item.equipe || item.qtd_equipe || item.qdt_equipe || item.equipes || 0);
    }
